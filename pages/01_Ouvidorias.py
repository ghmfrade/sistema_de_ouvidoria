...
if ouvidorias:
    ...
    with col_pag3:
        opcoes_pagina = [f"{i}/{num_paginas}" for i in range(1, num_paginas + 1)]
        pag_sel_str = st.selectbox("Página", opcoes_pagina, index=st.session_state.pag_atual - 1, key="pag_select")
        pag_num = int(pag_sel_str.split("/")[0])
